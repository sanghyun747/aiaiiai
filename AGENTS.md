...
- Ask before production DNS mutation, purchase, payment, public deployment, remote push or sending private material. A sandbox API call is not proof of public publication.
...
- Session A writes only frontend/**, including frontend/package.json, frontend/package-lock.json, frontend/tests/** and frontend/HANDOFF.md.
...
- No contract change by negotiation between executors. Propose it to the architect. Implement behind the existing v1 contract until both sides receive an approved revision.
...
- Raw follower diff categories remain preliminary until the audit gate. LLM prose cannot change deterministic counts or audit statuses.
