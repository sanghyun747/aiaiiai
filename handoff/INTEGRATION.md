...
- Both start at tag contracts-v1. All preparation files are present in both worktrees.
...
3. Verify both branches point to the SAME contracts-v1 baseline and both worktrees are clean.
...
Each executor runs, from their own project directory:
    ...
    python -X utf8 -B docs/verify_prep.py --base contracts-v1 --owner A
...
Use actual verified branch tips, not stale hashes from a previous session. Example Git Bash commands:
    ...
    git -C C:/Users/sangh/trendpilot-hackathon-20260919 diff --name-status contracts-v1..feat/frontend
    git -C C:/Users/sangh/trendpilot-hackathon-20260919 diff --name-status contracts-v1..feat/backend
...
After both checks pass, choose a new unused integration path and branch, then:
    ...
    git -C C:/Users/sangh/trendpilot-hackathon-20260919 worktree add -b integration/mvp C:/Users/sangh/trendpilot-hackathon-20260919-integration contracts-v1
...
Never use 'ours/theirs' to conceal a conflict. If a conflict occurs, retain the state, inspect ownership and fix specific content with review. Do not push or publish without user approval. No deletion of old worktrees afterwards.
