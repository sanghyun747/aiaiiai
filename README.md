...
**트렌드를 나열하는 챗봇이 아니라, 공개 콘텐츠 근거와 내 계정의 변화를 바탕으로 다음 콘텐츠 실험 3개를 설계하고 실행 자료를 만드는 도구.**

장기 방향은 트렌드 관측 → 콘텐츠 실행 → 성과 기록 → 다음 실험이다. 이번 MVP는 관측·팔로워 비교·실험 자료 생성까지 구현한다. 실제 게시, 성과 자동 수집, 학습 루프가 완성됐다고 발표하지 않는다.
...
| 실제 문제 해결 | 무엇을 만들지보다 넓은 '유행의 최전선' 목표 | '내 주제와 제작 시간에 맞춰 다음에 무엇을 찍을지' 해결 |
...
4. Nosana가 제공된 근거에 연결된 콘텐츠 실험 3개를 생성한다. 각 실험은 도입부 A/B, 촬영 구성, 캡션, 제작 시간, 전환 문구, 측정 지표를 포함한다.
5. Daytona가 고정된 안전한 코드로 분석과 Markdown 보고서 생성을 실제 수행한다.
6. 결과 화면에서 출처, 한계, 스폰서 실행 증거를 보고 보고서를 내려받는다.
7. DNSimple의 별도 게시 동작으로 레코드를 생성/확인한다. sandbox 성공은 공개 URL 성공과 다르게 표시한다.

UI는 한 대시보드 안의 세 영역으로 제한한다: 관측 근거 / 팔로워 변화 / 다음 실험. 별도 로그인·결제·관리자·영상 생성 화면은 만들지 않는다.
...
→ Nosana: 비식별 집계 + 출처 목록으로 실험 JSON 작성
→ Daytona: 고정 템플릿으로 Markdown 산출물 생성
...
- 세션 A: `frontend/**`만 소유. React/TypeScript/Vite UI, HTTP 클라이언트, 명시적 mock, 화면 테스트, 로컬 실행 문서.
...
1:15–2:00: 근거 있는 실험 3개와 촬영 자료·측정 지표. 결과가 아직 없으므로 성장 효과는 주장하지 않는다.
2:00–2:30: Nosana/Daytona 실행 기록, 생성한 Markdown 다운로드, DNSimple sandbox 레코드 또는 검증된 공개 URL.
